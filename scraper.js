...
const { EBANKING_URL, EBANKING_USERNAME, EBANKING_PASSWORD } = process.env

const get_transactions_from_table = async (page) =>
  page.evaluate(() => {
    var transactions = []
...
exports.scrape = async () => {
  ...
  await page.evaluate((EBANKING_USERNAME) => {
    document.querySelector("input[name='ctl00$cphBizConf$txtLoginId']").value =
      EBANKING_USERNAME
  ...
  await page.evaluate((EBANKING_PASSWORD) => {
    document.querySelector('input[name="ctl00$cphBizConf$txtLoginPw"]').value =
      EBANKING_PASSWORD
